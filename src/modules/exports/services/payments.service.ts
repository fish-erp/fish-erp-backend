...
export class PaymentsService {
  ...
  async add(id: string, input: PaymentInput, actor: string) {
    ...
    await this.prisma.$transaction(async tx => {
      ...
      if (invoice.exportStatus !== 'COMPLETED' || !invoice.paymentTracked) throw new BadRequestException('Chỉ thu tiền phiếu hoàn tất đã xác định thanh toán');
      ...
      await tx.invoicePayment.create({ data: { invoiceId: id, amount, idempotencyKey: input.idempotencyKey, note: input.note?.trim() || null, ...(input.paidAt ? { paidAt: new Date(input.paidAt) } : {}), createdBy: actor } });
    });
  ...
  async reverse(id: string, paymentId: string, input: ReversalInput, actor: string) {
    ...
    await this.prisma.$transaction(async tx => {
      await lockInvoice(tx, id);
      const payment = await tx.invoicePayment.findFirst({ where: { id: paymentId, invoiceId: id } });
      if (!payment) throw new BadRequestException('Khoản thu không tồn tại');
      if (payment.reversedAt) return;
      await tx.invoicePayment.update({ where: { id: paymentId }, data: { reversedAt: new Date(), reversedBy: actor, reversalReason: input.reason.trim() } });
    });
    return this.exports.findById(id);
  }
  async reconcile(id: string, input: ReconcileInput, actor: string) {
    ...
    await this.prisma.$transaction(async tx => {
      ...
      if (invoice.paymentTracked) throw new ConflictException('Phiếu đã xác định thanh toán; hãy kiểm tra lịch sử thu');
      const total = invoiceTotal(invoice.exportProducts);
      ...
      await tx.exportInvoice.update({ where: { id }, data: { customerId, paymentTracked: true, reconciliationNote: input.note.trim(), reconciledAt: new Date(), reconciledBy: actor, updatedBy: actor } });
      if (input.paidAmount > 0) await tx.invoicePayment.create({ data: { invoiceId: id, amount: input.paidAmount, note: 'Đối soát đơn cũ: ' + input.note.trim().slice(0,470), idempotencyKey: 'reconcile:' + id, createdBy: actor } });
    });
    return this.exports.findById(id);
  }
}
